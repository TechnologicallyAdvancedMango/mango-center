...
import { updateWorld, getVoxelGlobal, BLOCKS, isChunkLoaded } from "./main.js";
...
for (const name in TEXTURES) {
    MATERIALS[name] = new THREE.MeshStandardMaterial({ map: TEXTURES[name] });
}
...
// Chunk border wireframe
const chunkBorderGeo = new THREE.EdgesGeometry(new THREE.BoxGeometry(16, 16, 16));
const chunkBorderMat = new THREE.LineBasicMaterial({ color: 0xffaa00 });
export const chunkBorders = [];
...
export function addChunkBorder(cx, cy, cz, size) {
    ...
    border.position.set(cx * size + size / 2, cy * size + size / 2, cz * size + size / 2);
        ...
        for (x[d] = -1; x[d] < size; ) {
            ...
            for (let j = 0; j < size; j++) {
                for (let i = 0; i < size; ) {
                    ...
                    let uAxis, vAxis;
                    if (d === 0) { uAxis = 2; vAxis = 1; }
                    else if (d === 1) { uAxis = 0; vAxis = 2; }
...
window.addEventListener("mousedown", (e) => {
    ...
    if (e.button === 0) {
        window.dispatchEvent(new CustomEvent("game:requestBreak", { detail: { x: hit.break.x, y: hit.break.y, z: hit.break.z } }));
    }

    if (e.button === 2) {
        window.dispatchEvent(new CustomEvent("game:requestPlace", { detail: { x: hit.place.x, y: hit.place.y, z: hit.place.z } }));
    }

    if (e.button === 1) {
        window.dispatchEvent(new CustomEvent("game:requestPick", { detail: { x: hit.break.x, y: hit.break.y, z: hit.break.z } }));
    }
});
...
export function startRenderLoop() {
    function animate() {
        ...
        sun.position.set(camera.position.x + 20, camera.position.y + 40, camera.position.z + 20);
